...
namespace CareerMonitoring.Infrastructure.Repositories {
    public class EmployerRepository : IEmployerRepository {
        ...
        public async Task<Employer> GetByEmailAsync (string email, bool isTracking = true) {
            if (isTracking)
                return await _context.Employers.AsTracking ().SingleOrDefaultAsync (x => x.Email == email);
            return await _context.Employers.AsNoTracking ().SingleOrDefaultAsync (x => x.Email == email);
        }
