...
namespace CareerMonitoring.Infrastructure.Repositories {
    public class StudentRepository : IStudentRepository {
        ...
        public async Task<Student> GetByEmailAsync (string email, bool isTracking = true) {
            if (isTracking)
                return await _context.Students.AsTracking ().SingleOrDefaultAsync (x => x.Email.ToLowerInvariant() == email.ToLowerInvariant());
            return await _context.Students.AsNoTracking ().SingleOrDefaultAsync (x => x.Email.ToLowerInvariant() == email.ToLowerInvariant());
        }
