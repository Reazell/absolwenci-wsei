...
namespace CareerMonitoring.Infrastructure.Repositories
{
    public class FieldDataRepository : IFieldDataRepository
    {
        ...
        {
            if(isTracking)
                return await _context.FieldData.AsTracking().Include(x => x.ChoiceOptions).Include(x => x.Rows).SingleOrDefaultAsync (x => x.Id == id);
            return await _context.FieldData.AsNoTracking().Include(x => x.ChoiceOptions).Include(x => x.Rows).SingleOrDefaultAsync (x => x.Id == id);
        }
